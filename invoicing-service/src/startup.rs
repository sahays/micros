...
impl Application {
    ...
    async fn build_internal(
        ...
        // Try to connect to ledger service (optional - graceful degradation if unavailable)
        let ledger_client = match LedgerClient::connect(&config.ledger_service.url).await {
            Ok(client) => {
                tracing::info!(
                    ledger_service_url = %config.ledger_service.url,
                    "Connected to ledger service"
                );
                Some(Arc::new(client))
            }
            Err(e) => {
                tracing::warn!(
                    ledger_service_url = %config.ledger_service.url,
                    error = %e,
                    "Failed to connect to ledger service - ledger integration disabled"
                );
                None
            }
        };
        ...
        tracing::info!(
            http_port = http_port,
            grpc_port = grpc_port,
            "Invoicing service listeners bound"
        );
    ...
    pub async fn run_until_stopped(self) -> std::io::Result<()> {
        ...
        // Build gRPC server with optional ledger client
        let invoicing_service = match self.state.ledger_client {
            Some(ref ledger_client) => InvoicingServiceImpl::with_ledger_client(
                self.state.db.clone(),
                ledger_client.clone(),
            ),
        ...
        health_reporter
            .set_serving::<InvoicingServiceServer<InvoicingServiceImpl>>()
            .await;
