//! Capability enforcement tests for document-service.
//!
...
mod capability_checker_tests {
    ...
    async fn disabled_checker_allows_all_requests() {
        ...
        let result = checker
            .require_capability(&request, "document:upload")
            .await;
    ...
    async fn disabled_checker_returns_auth_context_from_headers() {
        ...
        let result = checker.require_capability(&request, "document:read").await;
        assert!(result.is_ok());
...
mod capability_constants_tests {
    use document_service::grpc::capability_check::capabilities;

    #[test]
    fn document_capabilities_are_defined() {
        assert_eq!(capabilities::DOCUMENT_UPLOAD, "document:upload");
        assert_eq!(capabilities::DOCUMENT_DOWNLOAD, "document:download");
        assert_eq!(capabilities::DOCUMENT_READ, "document:read");
        assert_eq!(capabilities::DOCUMENT_DELETE, "document:delete");
        assert_eq!(capabilities::DOCUMENT_PROCESS, "document:process");
        assert_eq!(
            capabilities::DOCUMENT_SIGNED_URL,
