use document_service::config::{DocumentConfig, StorageBackend};
...
async fn upload_document_works() {
    ...
    config.storage.local_path = Some(format!("target/test-storage-{}", Uuid::new_v4()));
    ...
    let form = multipart::Form::new()
        .part("file", multipart::Part::bytes(vec![0; 100])
            .file_name("test.txt")
    ...
    // 5. Verify Storage
    let storage_path = std::path::Path::new(config.storage.local_path.as_ref().unwrap())
        .join(&stored_doc.s3_key);
    assert!(storage_path.exists());

    // Cleanup
    let _ = db.client().database(&db_name).drop(None).await;
    let _ = tokio::fs::remove_dir_all(config.storage.local_path.unwrap()).await;
}
