...
async fn activate_trial_subscription_works() {
    ...
    let subscription = create_test_subscription(&mut client, app.tenant_id(), true).await;
...
async fn activate_active_subscription_fails() {
    ...
    let subscription = create_test_subscription(&mut client, app.tenant_id(), false).await;
