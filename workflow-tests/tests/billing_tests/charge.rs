...
async fn create_one_time_charge_works() {
    ...
    let (subscription, _cycle_id) = create_test_subscription(&mut client, app.tenant_id()).await;
...
async fn list_charges_works() {
    ...
    let (subscription, cycle_id) = create_test_subscription(&mut client, app.tenant_id()).await;
