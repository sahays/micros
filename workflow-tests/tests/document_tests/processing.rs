...
async fn manual_processing_trigger_works() {
    ...
    let get_response = client
        .get_document(TEST_APP_ID, TEST_ORG_ID, &user_id, doc_id.clone())
        .await
    ...
    let process_response = client
        .process_document(TEST_APP_ID, TEST_ORG_ID, &user_id, doc_id.clone(), None)
        .await
...
async fn processing_nonexistent_document_returns_not_found() {
    ...
    let status_result = client
        .get_processing_status(TEST_APP_ID, TEST_ORG_ID, &user_id, fake_id.clone())
        .await;
    ...
    let process_result = client
        .process_document(TEST_APP_ID, TEST_ORG_ID, &user_id, fake_id, None)
        .await;
...
async fn any_authenticated_caller_can_process_document() {
    ...
    let user_id_owner = Uuid::new_v4().to_string();
    let user_id_service = Uuid::new_v4().to_string();
