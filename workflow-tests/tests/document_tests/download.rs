...
async fn download_document_works() {
    ...
    let (filename, content_type, downloaded_data) = client
        .download_document(TEST_APP_ID, TEST_ORG_ID, &user_id, doc_id)
        .await
...
async fn download_nonexistent_document_returns_not_found() {
    ...
    let result = client
        .download_document(TEST_APP_ID, TEST_ORG_ID, &user_id, fake_id)
        .await;
...
async fn delete_document_works() {
    ...
    // Delete the document
    let response = client
        .delete_document(TEST_APP_ID, TEST_ORG_ID, &user_id, doc_id.clone())
        .await
    ...
    // Verify it's gone
    let result = client
        .get_document(TEST_APP_ID, TEST_ORG_ID, &user_id, doc_id)
        .await;
