...
async fn create_assignment_client(addr: &str) -> AssignmentServiceClient<Channel> {
...
async fn user_with_specific_capability_can_access_endpoint() {
    ...
    use workflow_tests::proto::auth::AssignCapabilityRequest;
    role_client
    ...
    let mut assignment_client = create_assignment_client(&app.grpc_addr()).await;
    assignment_client
