...
impl TestApp {
    /// Connect to the deployed billing-service.
    ...
    /// Create a gRPC client connected to the deployed billing-service.
    pub async fn grpc_client(&self) -> BillingServiceClient<Channel> {
        BillingServiceClient::connect(self.grpc_address.clone())
            .await
            .expect("Failed to connect to billing gRPC server")
    }
